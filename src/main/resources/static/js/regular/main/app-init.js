...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderNotifications() {
        if (!notificationList || typeof notificationData === 'undefined' || typeof createNotificationCard !== 'function') return;
    ...
    function renderFavoriteProperties() {
        if (!favoriteList || typeof favoriteProperties === 'undefined' || typeof createFavoritePropertyCard !== 'function') return;
    ...
    function renderCompareGroups() {
        if (!compareList || typeof compareGroups === 'undefined' || typeof createCompareCard !== 'function') return;
    ...
    function renderChatList(data) {
        if (!chatListContainer || typeof chatData === 'undefined' || typeof createChatCard !== 'function') return;
    ...
    function initializeChatSearch() {
        const searchInput = document.querySelector('#chat-panel input[placeholder="채팅방 검색"]');
        if (searchInput) {
    ...
    // 알림: 모든 알림 읽음 처리
    document.getElementById('mark-all-read-button')?.addEventListener('click', () => {
    ...
    // 비교: 새 그룹 추가
    document.getElementById('add-compare-group-button')?.addEventListener('click', () => {
         if(typeof compareGroups !== 'undefined') {
            const newGroupId = compareGroups.length > 0 ? Math.max(...compareGroups.map(g => g.groupId)) + 1 : 1;
    ...
    // 내 매물: 새 매물 등록
    document.getElementById('add-new-property-button')?.addEventListener('click', () => {
        alert("새 매물 등록 페이지로 이동합니다.");
    });
    
    // 프로필 패널 기능
    if(profilePanel) {
        profilePanel.querySelector('button[type="button"]')?.addEventListener("click", () => {
